...
class CallNewPage extends StatefulWidget {
  ...
  @override
  _CallNewPageState createState() => _CallNewPageState();
}

class _CallNewPageState extends State<CallNewPage> with WidgetsBindingObserver {
  final _users = <int>[];
  ...
  List<ViewerModel>? guestLive;
  initializeViewerLive() async {
    guestLive=  FirestoreServices().getViewerLive( widget.channelName??"") as List<ViewerModel>;
  }
  ...
  Future<void> initializeForBroadcaster() async {
    ...
    await _engine!.setClientRole(role: ClientRoleType.clientRoleBroadcaster,options: ClientRoleOptions());
  ...
  void _addAgoraEventHandlers() {
    _engine!.registerEventHandler(RtcEngineEventHandler(
      ...
      setState(() {
        ...
        onJoinChannelSuccess: (RtcConnection channel,int elapsed) {
      print(elapsed);
      setState(() {
        final info = 'onJoinChannel: $channel, uid: $elapsed';
        _infoStrings.add(info);
        setState(() {
          _joined = true;
        });
  ...
  _getRenderViews() {
    ...
    List<Widget> guests = [];
    // if (guest && Viewers[0].uid == user.userId) {
    //   isMute= Viewers[0].isMute ;
    //   isCameraShown = Viewers[0].isCamera;
    //   broadcasterAudioMute = Viewers[0].broadcaster_mute;
    //   broadcasterVideoMute = Viewers[0].broadcaster_video_mute;
    // }
    ...
    for (int i = 0; i <Viewers.length;i++) {
      _users.forEach((int uid) {
        if (uid != 10010 && Viewers.length != 0) {
          ...
          if(isNext){
          setState(() {
            if(Viewers[0].uid == user!.userId!){
              isMute= Viewers[0].isMute! ;
              isCameraShown = Viewers[0].isCamera!;
              broadcasterAudioMute = Viewers[0].broadcaster_mute!;
              broadcasterVideoMute = Viewers[0].broadcaster_video_mute!;
            }
            guest && Viewers[i].uid == user?.userId ?
              ...
              child: Stack(
                children: [
                  isCameraShown && !broadcasterVideoMute
                      ...
                      : Image.network(user?.image??"", width: 300, fit: BoxFit.cover),
                  Align(
                                ...
                                onPressed: () async {
                                  if(!broadcasterAudioMute) {
                                    ...
                                    FirestoreServices.joinUserMuteUnmute(
                                        widget.channelName??"", isMute,
                                        Viewers[i].uid??"");

                                  }else{
                                    Fluttertoast.showToast(msg: "Sorry! You can't unmute audio to yourself now");
                                  }
                                },
                                icon: Icon(
                                  isMute  || broadcasterAudioMute? Icons.mic_off_rounded : Icons.mic,
                                  color: Colors.white,
                                )),
                            IconButton(
                                onPressed: () {
                                  if(!broadcasterVideoMute) {
                                    ...
                                    _engine?.muteLocalVideoStream(isCameraShown);
            ...
            guests.add(Padding(
              padding: EdgeInsets.only(
                  bottom: _users.length == 1 ? 5.0 : 5.0),
              child: Container(
                height: 130,
                width: 130,
                child: Viewers[i].isCamera!
                    ? RtcRemoteView.SurfaceView(uid: uid)
                    : Viewers.length != 0
                    ? Image.network(
                  Viewers[i].photoUrl??"", width: 130, height: 130,)
                    : Image.asset('images/profile.png', height: 130,
                    width: 130,
                    fit: BoxFit.cover),
              ),
            ),);
      ...
      child: FractionallySizedBox(
        ...
        child: Column(
          ...
          children: [
            Expanded(
              child: Container(
                padding: EdgeInsets.symmetric(vertical: 2.h.w),
                child: StreamBuilder(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    !chat.name!.contains('null')
                                      ...
                                      onTap: () async {
                                        ...
                                        showModalBottomSheet<void>(
                                          ...
                                          builder: (BuildContext context) {
                                            return UserProfileDailog(userModel:userModel,
                                                status:status,
                                      ...
                                      child:  Container(
                                          height: 20, width: MediaQuery.of(context).size.width * 0.14, padding: EdgeInsets.zero,
                                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), gradient: LinearGradient(begin: Alignment.centerRight, end: Alignment.centerLeft,
                                            colors: [
                                              Color(int.parse("${chat.rlColorCode?.replaceFirst("#", "0xff")}")),
                                              Colors.black87
                                            ]
                                            ,)),
                                          child:Row(
                                            ...
                                            children: [
                                              CircleAvatar(
                                                radius: 8,
                                                backgroundImage: NetworkImage(chat.rlIcon??""),
                                              ),
                                    ...
                                    chat.giftUrl != null &&  !chat.giftUrl!.contains('null')? Image.network(chat.giftUrl??"", height: 25.h, width: 25.w,):Container(),
                                  ],
                                ),
                              ),
                            ],
          ...
          if (q.docs.length > 0) {
            q.docs.forEach((d) async {
              ...
              if (user?.userId == d.id &&
                ...
                _engine?.muteLocalAudioStream(broadcasterAudioMute);
                _engine?.muteLocalVideoStream(broadcasterVideoMute);
              ...
              builder: (BuildContext context,
                ...
                if (snapshot.hasData && snapshot.data!.exists) {
                  ...
                  if (snapshot.data?.get('time') == giftTime) {
                    ...
                    if (snapshot.data!.get('gifturl').toString().isNotEmpty && !snapshot.data!.get('gifturl')
                            .toString()
                            .contains("mp4")) {
                                ...
                                child: Stack(
                                  children: [
                                    AnimatedAlign(
                                      ...
                                      child: snapshot.data!.get('gifturl')
                                              .toString()
                                              .contains("mp4")
                                          ? Container(
                                              child: Container(
                      ...
                      Container(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              SizedBox(height: 10,),
                              Text('You have to invite this call. '
                                  'Are you join call?', style: TextStyle(
                                  color: Colors.black,
                                  fontSize: 20.sp,
                                  ...
                                  Expanded(flex: 1 ,
                                    child: GestureDetector(onTap: () async {
                                      await FirebaseFirestore.instance
                                          .collection('live_streams')
                                          .doc(widget.channelName)
                                          .collection("invite").doc(user?.userId).delete();

                                      FirestoreServices.addUserToVideoLive(1, widget.channelName??"",snapshot.data?.get("uid"));
                                      guest = true;
                                      await _engine?.setClientRole(ClientRole.Broadcaster);
                                      broadcasterAudioMute = snapshot.data?.get("broadcaster_mute");
                                      broadcasterVideoMute = snapshot.data?.get("broadcaster_video_mute");
                                      _engine?.muteLocalAudioStream(broadcasterAudioMute);
                                      _engine?.muteLocalVideoStream(broadcasterVideoMute);
                                      ...
                                      child: Container(
                                        ...
                                        child: Text(
                                          "Join Call", textAlign: TextAlign.center,
                                          style: TextStyle(
                                              color: Colors.white,
                                  ...
                                  Expanded(
                                    ...
                                    child: GestureDetector(onTap: () async {
                                      var collection = FirebaseFirestore.instance
                                          ...
                                          .doc(widget.channelName)
                                          .collection("invite");
                                      var snapshot = await collection.where('uid', isEqualTo: user?.userId).get();
            ...
            child: Center(
              child: Stack(
                children: <Widget>[
                          ...
                          child: Row(
                            children: [
                                                              ...
                                                              children: [
                                                                ...
                                                                GestureDetector(
                                                                  onTap: () async {
                                                                    ...
                                                                    await Share.share("https://play.google.com/store/apps/details?id=com.lovello.lovello",
                                                                        subject: "HDLive",
                                                                        sharePositionOrigin: box.localToGlobal(Offset.zero) & box.size);
                                                                  },
                                                                  child: Padding(padding: EdgeInsets.only(left: 10.h.w, top: 0.h.w, right: 10.h.w),
                                                                    child: Container(height: 25.h, width: 25.w,
                                                                      child: Image.asset("images/forward.png", color: Colors.red,),
                                                                    ),
                                                                  ),
                                                                ),
                                                              ],
                                                              ...
                                                              children: [
                                                                ...
                                                                Expanded(
                                                                  ...
                                                                  child:
                                                                      TabBarView(
                                                                    children: [
                                                                      VideoLiveWaitingList(
                                                                        channelName: widget.channelName??"",
                                                                        broadcaster: widget.broadcaster??false,
                                                                      ...
                                                                      GuestList(
                                                                        ...
                                                                        broadcaster: widget.broadcaster,
                                                                          isVideo:true,
                                                                        callEnd:() async {
                                                                          _onCallEndUser(context);
                                                                        },
                                                                      )
                                                                    ],
                                                                  ),
                                                                )),
                                                                widget.broadcaster??false
                                                                    ? Container()
                                                                    : AudioVideoJoin(
                                                                        channelName:
                                                                            widget.channelName??"",
                              ...
                              IconButton(
                                  onPressed: () async {
                                    if(broadcasterLeft){
                                      Fluttertoast.showToast(msg: 'Sorry! You can not send gift.');
                                      return;
                                    }
                                    var i =
                  ...
                  LiveHeader(
                      isAudio: false,
                      broadcastUser: widget.broadcastUser??LiveModel(),
                      broadcaster: widget.broadcaster??false,
                      channelName: widget.channelName??"",
  ...
  void _setVideoBeauti(int j) {
    if (j == 0) {
      _engine?.setBeautyEffectOptions(
          true,
          BeautyOptions(
              lighteningContrastLevel: LighteningContrastLevel.Low,
              lighteningLevel: 0.0,
              smoothnessLevel: 0.0,
              rednessLevel: 0.0));
    } else if (j == 1) {
...
class VideoLiveWaitingList extends StatefulWidget {
  ...
  VideoLiveWaitingList({this.channelName, this.broadcaster, this.user});
                                    ...
                                    onPressed: () async {
                                      if(guestLive < 2) {
                                        FirestoreServices.addUserToVideoLive(
                                            viewer.status??0, widget.channelName??"",viewer.uid??"");
                                        isUserJoin = await getJoinUser(
                                            viewer, widget.user??CurrentUserModel());
                                        // setState(() {});
                                        print(
...
class Dependencies {
  ...
  final TextStyle textStyle = const TextStyle(fontSize: 17.0,);
...
class TimerText extends StatefulWidget {
  ...
  TimerTextState createState() => new TimerTextState(dependencies: dependencies??Dependencies());
}
...
class TimerTextState extends State<TimerText> {
  ...
  @override
  void initState() {
    timer = new Timer.periodic(new Duration(milliseconds: dependencies?.timerMillisecondsRefreshRate??0), callback);
    super.initState();
  }
  ...
  void callback(Timer timer) {
    if (milliseconds != dependencies?.stopwatch.elapsedMilliseconds) {
      ...
      final int hundreds = (milliseconds??0 / 10).truncate();
    ...
    return new Row(
      ...
      children: <Widget>[
         RepaintBoundary(
          child:  SizedBox(
            height: 20.0,
            child:  MinutesAndSeconds(dependencies: dependencies??Dependencies()),
          ),
        ),
      ],
    );
...
class MinutesAndSeconds extends StatefulWidget {
  ...
  MinutesAndSecondsState createState() => new MinutesAndSecondsState(dependencies: dependencies??Dependencies());
}
