...
class _AudioLiveState extends State<AudioLive>
  ...
  initializeViewerStream() async {
    ...
    /*Broadcaster Remove from seat */
    FirebaseFirestore.instance
        .collection("live_streams")
        .doc(widget.channelName)
        .collection("viewers")
        .where('status', whereIn: [6])
        .snapshots()
        .listen((QuerySnapshot q) {
          if (q.docs.length > 0) {
  ...
  Future<void> _initAgoraRtcEngine() async {
    ...
    // int volume = 50;
    // await _engine.setInEarMonitoringVolume(volume);
    ...
    if (widget.broadcaster!) {
      // await _engine!.joinChannel(Token, widget.channelName ?? "", null, 10010, token: '', channelId: '');
      ...
      RtcEngineEventHandler(
        ...
        onJoinChannelSuccess: (RtcConnection connection, int elapsed) {
          ...
          setState(() {
            ...
            int uid = int.parse(connection.channelId!);
            _infoStrings.add(info);
            _userMap.addAll(
              {
                uid: User(int.parse(connection.channelId!), false),
              },
            );
          });
        },
        ...
        onUserJoined: (RtcConnection connection, int remoteUid, int elapsed) {
          int uid = int.parse(connection.channelId!);
          setState(() {
            print("userJoined: uid: ${connection.channelId}");
            final info = 'userJoined: ${connection.channelId}';
            _infoStrings.add(info);
        ...
        onUserOffline: (RtcConnection connection, int remoteUid,UserOfflineReasonType reason) {
          setState(() {
            ...
            if (connection.channelId == 10010) {
              broadcasterLeft = true;
              _infoStrings.add(info);
              _users.remove(connection.channelId);
              _userMap.remove(connection.channelId);
            }
          });
        },
        //// RtcConnection connection, int remoteUid, int width,int height, int elapsed
        onFirstRemoteVideoFrame: (RtcConnection connection,int remoteUid,int width,int height, elapsed) {
          setState(() {
                ...
                return Container(
                  ...
                  child: Column(
                    children: [
                        ...
                        children: [
                          ...
                          Center(
                            child: Padding(
                              ...
                              child: snapshot.data!.get("photoUrl") != ""
                                  ? CircleAvatar(
                                      child: CircleAvatar(
                      ...
                      //     : InitIconContainer(radius: MediaQuery.of(context).size.width * .26, text: snapshot.data.get("name"),),
                      Padding(
          ...
          pipmode
              ...
              : StreamBuilder(
                  stream: FirestoreServices()
                      .getUserSeatList(widget.channelName ?? ""),
                  builder: (context, snapshot) {
                    listViewer.clear();
                    emojiViewers.clear();
                    if (snapshot.hasData) {
                      List<ViewerModel>? viewers =
                          snapshot.data as List<ViewerModel>;
                      for (var i = 0; i < 8; i++) {
                      ...
                      if (viewers.length != 0) {
                        for (var i = 0; i < viewers.length; i++) {
                          ...
                          if (viewers[i].uid == user!.userId) {
                            onNewUserMuteListen(
                                viewers[i].isMute!, viewers[i].uid!);
                          }
                                    ...
                                    children: [
                                      ...
                                      chat.name!.contains('null') &&
                                          ...
                                          : !chat.name!.contains('null')
                                              ...
                                              : TextSpan(
                                                  text:
                                                      'New user joined this room, come & chat together !',
                                                  style: TextStyle(
                                                      color: Colors.white,
                                                      fontSize: 12)),
                                    ],
                                                                  ...
                                                                  children: [
                                                                                ...
                                                                                return StatefulBuilder(builder: (context, state) {
                                                                                  return DraggableScrollableSheet(
                                                                                    ...
                                                                                    builder: (context, scrollController) {
                                                                                      return Container(
                                                                                        decoration: BoxDecoration(color: Colors.black87, borderRadius: BorderRadius.only(topLeft: Radius.circular(30.r), topRight: Radius.circular(30.r))),
                                                                                        child: Column(
                                                                                          crossAxisAlignment: CrossAxisAlignment.start,
                                                                                          children: [
                                                                                            ...
                                                                                            Expanded(
                                                                                                child: FutureBuilder<EmojiModel>(
                                                                                              future: EmojiService().getEmoji().then((emoji) => emoji ?? EmojiModel()),
                                                                                              builder: (context, snapshot) {
                                                                                                if (snapshot.hasData) {
                                                                                                  return Padding(
                                                                                                    ...
                                                                                                    child: Container(
                                                                                                      child: GridView.builder(
                                                                                                        ...
                                                                                                        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 1, childAspectRatio: 2, crossAxisSpacing: 15, mainAxisSpacing: 15),
                                                                                                        scrollDirection: Axis.horizontal,
                                                                    ...
                                                                    IconButton(
                                                                        onPressed:
                                                                          ...
                                                                          if (widget
                                                                              .broadcaster!) {
                                                                            _engine!.muteLocalAudioStream(ismute);
                                                                            FirestoreServices.onMute(widget.channelName ?? "",
                                                                                ismute);
                                                                          }
                                                                        },
                                                                        icon:
                                                                                              ...
                                                                                              Row(
                                                                                                ...
                                                                                                children: [
                                                                                                  GestureDetector(
                                                                                                    ...
                                                                                                    child: Padding(
                                                                                                      padding: EdgeInsets.only(left: 20.h.w, top: 10.h.w, right: 10.h.w),
                                                                                                      child: Container(
                                                                                                        height: 25.h,
                                                                                                        width: 25.w,
                                                                                                        child: Image.asset(
                                                                                                  ...
                                                                                                  GestureDetector(
                                                                                                    onTap: () async {
                                                                                                      ...
                                                                                                      imagePaths.add("images/hdlive.jpg");

                                                                                                      final box = context.findRenderObject() as RenderBox;
                                                                                                      await Share.share("https://play.google.com/store/apps/details?id=com.lovello.lovello", subject: "HDLive", sharePositionOrigin: box.localToGlobal(Offset.zero) & box.size);
                                                                                                    },
                                                                                                    child: Padding(
                                                                                                      padding: EdgeInsets.only(left: 20.h.w, top: 10.h.w, right: 10.h.w),
                                                                                                      child: Container(
                                                                    ...
                                                                    IconButton(
                                                                                      ...
                                                                                      return Container(
                                                                                          ...
                                                                                          child: DefaultTabController(
                                                                                            ...
                                                                                            child: Column(
                                                                                              children: [
                                                                                                Container(
                                                                                                  padding: EdgeInsets.only(left: 50.h.w),
                                                                                                ...
                                                                                                Expanded(
                                                                                                    child: Container(
                                                                                                  padding: EdgeInsets.only(left: 20.h.w),
                                                                                                  child: TabBarView(
                                                                                                    children: [
                                                                                                      AudioWaitList(
                                                                        ...
                                                                        icon: badges.Badge(
                                                                            ...
                                                                            showBadge: requester != 0 ? true : false,
                                                                            child: Image.asset("images/Join_live_list.png"))),
                                                                    IconButton(
                                                                        onPressed:
                                                                            () {
                                                                          setState(
                                                                                        ...
                                                                                        return Container(
                                                                                          ...
                                                                                          child: Column(
                                                                                            ...
                                                                                            children: [
                                                                                              ...
                                                                                              Expanded(
                                                                                                  child: FutureBuilder<EmojiModel>(
                                                                                                future: EmojiService().getEmoji().then((e)=>e??EmojiModel()),
                                                                                                builder: (context, snapshot) {
                                                                                                  if (snapshot.hasData) {
                                                                                                    return Padding(
                                                                                                      padding: const EdgeInsets.all(8.0),
                                                                                ...
                                                                                builder: (context) {
                                                                                  return StatefulBuilder(builder: (context, state) {
                                                                                    return DraggableScrollableSheet(
                                                                                        ...
                                                                                        builder: (context, scrollController) {
                                                                                          return Container(
                                                                                            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.only(topLeft: Radius.circular(30.r), topRight: Radius.circular(30))),
                                                                                            child: Row(
                                                                                              children: [
                                                                                                GestureDetector(
                                                                                                  onTap: () async {
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          children: [
                                                                            ...
                                                                            ButtonTheme(
                                                                              child: GestureDetector(
                                                                                onTap: () async {
                                                                                  print('Viewres UId  === ${widget.broadcastUser?.name??""}');
                                                                                  if (!isJoin) {
                                                                                    await FirestoreServices.joinVideo(
                                                                                      1,
                                                                                      widget.channelName??"",
                                                                                    );
                                                                                    setState(() {
                                                                                        ...
                                                                                        return AlertDialog(
                                                                                          ...
                                                                                          actions: <Widget>[
                                                                                            ...
                                                                                            TextButton(
                                                                                              ...
                                                                                              child: Text('Yes', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.red)),
                                                                                            ),
                                                                                          ],
                                                                                        );
                                                                                      },
                                                                                      ...
                                                                                      children: [
                                                                                        Container(
                                                                                          ...
                                                                                          child: Image.asset(
                                                                                            ...
                                                                                            height: 25.h,
                                                                                            width: 35.w,
                                                                                            color: isJoin ? Colors.red : Colors.white,
                                                                                          ),
                                                                                        ),
                                                                                        Padding(
                                                                                          padding: const EdgeInsets.all(5.0),
                                                                                          child: Text(
                                                                                            'Raise',
                                                                                            style: TextStyle(color: !isJoin ? Colors.red.shade800 : Colors.transparent, fontSize: 11.0, fontWeight: FontWeight.w800),
                                                                                          ),
                                                                                        ),
                                                                                      ],
                                                                                    ),
  ...
  void getFiles() async {
    ...
    // files = await fm.filesTree(extensions: ["mp3"]  ); //optional, to filter files, list only mp3 files
    // setState(() {}); //update the UI
  }

  // Recursive function to fetch files with specific extensions
  Future<List<FileSystemEntity>> _fetchFiles(Directory directory, {List<String> extensions = const []}) async {
   ...
   showAudio() {
    ...
    showModalBottomSheet(
        context: context,
        builder: (context) {
          return AudioFiles(files: files?.whereType<File>().toList() ?? [], key: ValueKey(1),);
        });
  }
